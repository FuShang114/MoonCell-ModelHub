...
def generate_requests(duration, qps, seed=1):
    ...
    for sec in range(duration):
        ...
        for _ in range(n):
            ...
            if p < 0.5:
                ...
                out_tok = rng.randint(120, 360)
            elif p < 0.88:
                in_tok = rng.randint(180, 520)
                out_tok = rng.randint(360, 1300)
            else:
                in_tok = rng.randint(520, 1400)
                out_tok = rng.randint(1300, 3600)
            chunks = max(4, out_tok // 32)
            ttft = 0.035 + min(0.28, chunks * 0.0025) + rng.random() * 0.03
    ...
    while now <= end:
        ...
        while queue and total_inflight < policy.capacity():
            ...
            if not policy.on_admit(req):
                break
            # traditional random sample + min inflight
...
def run():
    qps_list = [40, 80, 120, 160, 200, 240]
    rows = []
    for i, qps in enumerate(qps_list, start=1):
        reqs = generate_requests(180, qps, seed=800 + i)
    ...
    csv_path = OUT / "qps_sweep_pool_vs_traditional.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        w.writeheader()
        w.writerows(rows)

    x = [str(r["qps"]) for r in rows]
    draw_svg(
        OUT / "qps_sweep_throughput.svg",
        "QPS Sweep - Throughput (RPM)",
        x,
    ...
    draw_svg(
        OUT / "qps_sweep_max_util.svg",
        "QPS Sweep - max(rpm_util,tpm_util)",
        x,
    ...
    draw_svg(
        OUT / "qps_sweep_gc.svg",
        "QPS Sweep - GC Frequency (/s)",
        x,
